...
#Creating the function to accept inputs and creating an 2nd array and predicting the resullt 

def predict(bedrooms,bathrooms,status,size,location,facing,Types):
    """Function To accpet the values"""
    selected_location = Location[location]
    selected_status= Status[status]
    selected_facing = Facing[facing]
    selected_types = Type[Types]
    user_input = np.array([[bedrooms,bathrooms,selected_status,size,selected_location,selected_facing,selected_types]])
    result = model.predict(user_input).round(2) 
    return result  
...
if __name__=="__main__":
    ...
    if submit_button:
        larger_text=f"<h2 style='color:cyan;'>The Predicted Price is: {result} Lakhs</h2>"
